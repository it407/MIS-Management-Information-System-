...
const AdminPendingTasks = () => {
        ...
        if (data.status === "success" && data.users) {
          ...
          const transformedTasks = data.users
            ...
            .map((user, index) => {
              ...
              // Calculate pending tasks (Today Task minus Total Achievement)
              const todayTask = parseInt(user['Today Task']) || 0;
              const totalAchievement = parseInt(user['Total Achievement']) || 0;
              // const pendingTaskCount = Math.max(0, todayTask - totalAchievement);
              const pendingTaskCount = parseInt(user['All Pending Till Date']) || 0;
              ...
              return {
                ...
                dueDate: user['End Date'] || new Date().toISOString().split('T')[0],
            ...
            .filter(task => task.pendingTaskCount > 0); // Only show tasks with pending items
          ...
          const uniqueEmployees = Array.from(
            new Map(
              transformedTasks.map(task => [task.personName, {
                ...
                email: user['Gmail ID'] || '', // Using Gmail ID from column J
  ...
  // Filter tasks based on all criteria - ONLY PENDING TASKS
  const filteredTasks = useMemo(() => {
    return enrichedTasks.filter(task => {
      // Only include tasks with pendingTaskCount > 0
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading pending tasks from Google Sheets...</p>
        </div>
      </div>
    );
  }
...
<div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
  ...
  <div className="rounded-lg shadow-sm p-4 border border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-sm font-medium text-blue-700">Total Persons</p>
  ...
  <div className="rounded-lg shadow-sm p-4 border border-green-200 bg-gradient-to-br from-green-50 to-green-100">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-sm font-medium text-green-700">Total FMS Names</p>
  ...
  <div className="rounded-lg shadow-sm p-4 border border-purple-200 bg-gradient-to-br from-purple-50 to-purple-100">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-sm font-medium text-purple-700">Total Task Names</p>
  ...
  <div className="rounded-lg shadow-sm p-4 border border-orange-200 bg-gradient-to-br from-orange-50 to-orange-100">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-sm font-medium text-orange-700">Total Pending Tasks</p>
  ...
  <div className="flex flex-col gap-3 md:flex-row md:gap-3">
    ...
    {/* Filters Row */}
    <div className="grid grid-cols-2 gap-3 w-full md:flex md:gap-3 md:w-auto">
      {/* Person Filter */}
        ...
        <div className="bg-white rounded shadow-sm">
          ...
          <div className="px-4 py-3 border-b border-gray-200">
            <h2 className="text-base font-semibold text-gray-700">
              Pending Tasks ({filteredTasks.length} found)
            </h2>
          </div>
